...
class Prius(BicycleModel):
    ...
    """
    def __init__(self, mode, spawn_offset: np.ndarray = np.zeros(3)):
        n = 2
        urdf_file = os.path.join(os.path.dirname(__file__), 'prius.urdf')
        self._scaling: float = 0.25
        super().__init__(n, urdf_file, mode)
        self._wheel_radius = 0.31265
        self._wheel_distance = 0.494
        # checking the size of the spawn offset
        assert spawn_offset.ndim == 1 and spawn_offset.shape[0] == 3
        self._spawn_offset: np.ndarray = spawn_offset
